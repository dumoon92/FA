...
for i in range(datensatzen):
    for l in range (datenlänge):
        
        inputdata.append(float(wave[i+l]))
inputdata = np.array(inputdata).reshape(datensatzen,datenlänge)
inputlabel = np.zeros(shape =(datensatzen,predictlen))
for i in range(datensatzen):    
    for j in range(predictlen):
        inputlabel[i,j] = wave[i+datenlänge+j]
xtest = []
for a in range(datenlänge):
    xtest.append(float(wave[testdatenstart+a]))
xtest = np.array(xtest).reshape(1,-1)     
ypredict = []
ytest = wave[testdatenstart+datenlänge:testdatenstart+datenlänge+predictlen]
